...
一个纯静态、轻量级的在线开发者工具箱，提供 104 个常用编解码、文本处理、格式化、生成器、转换器等实用工具。无需账号或后端服务，绝大多数数据只在浏览器本地处理。

## ✨ 特性

- **104 个实用工具** — 覆盖编码解码、哈希加密、文本处理、生成器、格式化、转换器、开发者参考、图片处理、网络工具、数学计算等 10 大分类
...
### 转换器（24 个）
时间戳转换 · 时间长度转换 · 进制转换 · 数字格式化 · 颜色转换 · 单位转换 · JSON↔YAML · TOML↔JSON · JSON↔XML · JSONL↔JSON · JSON Pointer 查询 · JSON Patch 生成 · JSON 转 CSV · CSV 转 JSON · JSONPath 查询 · Markdown 转 HTML · HTML 转 JSX · CSS 转 JS 对象 · JSON 对比 · 时区转换 · 日期计算器 · 中文大写金额 · 正则表达式可视化 · JSON 转 TypeScript
...
- `dist/tools/{tool-id}/index.html`：104 个工具的独立 SEO 落地页
...
测试覆盖工具注册表、全部 104 个工具的渲染冒烟测试、表单可访问名称、核心工具行为、Markdown XSS 防护、路由懒加载和主题偏好持久化。
