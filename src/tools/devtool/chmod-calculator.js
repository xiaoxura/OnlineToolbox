...
export default {
  ...
  render(container) {
    ...
    container.append(createElement('div', { className: 'form-group' }, [createElement('label', { className: 'label', textContent: '八进制权限' }), modeInput]), grid, createElement('div', { className: 'form-group' }, [createElement('label', { className: 'label', textContent: '符号权限' }), symbolic]), createElement('div', { className: 'result-box' }, [createElement('label', { className: 'label', textContent: 'chmod 命令' }), command, createCopyButton(() => command.value)]))
    updateChecks()
  }
}
